...
const LOCAL_IP = '192.168.0.106'; // ← change if needed
const PORT = 3001;

const BASE_URL =
  Platform.OS === 'android'
    ? `http://10.0.2.2:${PORT}/outfit-favorites`
    : `http://${LOCAL_IP}:${PORT}/outfit-favorites`;
...
export function useFavorites(userId: string) {
  ...
  const favoritesQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const outfitIds = response.data.map((outfit: any) => outfit.id); // ✅ assumes backend returns full outfit
      return outfitIds;
    },
  });
  ...
  return {
    favorites: favoritesQuery.data ?? [], // ✅ NEVER undefined
