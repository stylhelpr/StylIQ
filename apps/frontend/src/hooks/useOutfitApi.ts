// hooks/useOutfitApi.ts
import {useCallback, useEffect, useRef, useState} from 'react';
import {API_BASE_URL} from '../config/api';

export type OutfitApiItem = {
  index: number;
  id: string; // may be "<uuid>:image" or "<uuid>:text" from Pinecone
  label: string;
  image_url?: string;
...
/** Ensure we always surface a clean wardrobe UUID (strip ":image"/":text"). */
function cleanWardrobeId(raw?: string): string {
  if (!raw) return '';
  const parts = String(raw).split(':');
  return parts[0] || raw; // fallback to raw if first part is empty
}
...
export function apiItemToUI(
  ...
  const cleanId = cleanWardrobeId(item.id);
  ...
  return {
    id: cleanId, // ✅ use clean wardrobe UUID
