...
export const useVoiceControl = () => {
  ...
  const stopListening = async () => {
    log('🛑 stopListening() — full teardown');
    try {
      await Voice.stop();
      await Voice.cancel();
      await Voice.destroy();
      ...
      // 📌 Re-attach listeners so next session works
      setTimeout(() => attachListeners(), 300);

      // ⏱️ Commit fallback (ensures final speech is stored)
  ...
  return {
    ...
    stopListening, // ✅ Fully stop and teardown
  };
};
