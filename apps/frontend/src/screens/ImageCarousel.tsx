...
function Card({
  ...
  // Text spring animation: shifts left then snaps back when card becomes active
  const textShiftX = useSharedValue(0);

  useEffect(() => {
    if (isActive) {
      // Delay, then shift left and spring back smoothly
      const timeout = setTimeout(() => {
        textShiftX.value = -15;
        textShiftX.value = withSpring(0, {
          damping: 10,
          stiffness: 120,
          mass: 0.6,
        });
  ...
  // Blur effect - only applies when card scrolls UP and out of view (not when peeking from below)
  const blur = useDerivedValue(() => {
    const scrollPosition = scrollY.value;
    const cardScrollStart = index * CARD_HEIGHT;

    // Blur only when scrolling past this card (card moves up and out)
    // No blur when card is coming in from below (peeking)
    const blurAmount = interpolate(
      scrollPosition,
      [cardScrollStart, cardScrollStart + CARD_HEIGHT * 0.5],
      [0, 60],
      // [0, 50],
      Extrapolation.CLAMP,
    );
  ...
  // Scale effect - only when card scrolls out
  const scale = useDerivedValue(() => {
    const scrollPosition = scrollY.value;
    const cardScrollStart = index * CARD_HEIGHT;

    const scaleAmount = interpolate(
      scrollPosition,
      [cardScrollStart, cardScrollStart + CARD_HEIGHT],
      [1, 0.95],
      Extrapolation.CLAMP,
    );
  ...
  // Opacity effect - only when card scrolls out
  const opacity = useDerivedValue(() => {
    const scrollPosition = scrollY.value;
    const cardScrollStart = index * CARD_HEIGHT;

    const opacityAmount = interpolate(
      scrollPosition,
      [cardScrollStart, cardScrollStart + CARD_HEIGHT],
      [1, 0.6],
      Extrapolation.CLAMP,
  ...
  const textAnimatedStyle = useAnimatedStyle(() => {
    ...
    return {
      ...
      transform: [{translateX: textShiftX.value}],
    };
  });

  return (
    <Animated.View style={[styles.card, animatedStyle]}>
      {/* Skia Canvas with blur effect */}
      <Canvas style={styles.skiaCanvas}>
        {skiaImage && (
          <SkiaImage
            ...
            origin={{x: width / 2, y: (CARD_HEIGHT + 100) / 2}}>
            <Blur blur={blur} />
          </SkiaImage>
        )}
      </Canvas>
