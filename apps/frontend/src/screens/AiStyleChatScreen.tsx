...
//   //       a.volume = 1.0;
//   //       a.onplaying = () => window.ReactNativeWebView.postMessage('playing');
//   //       a.onended  = () => window.ReactNativeWebView.postMessage('ended');
//   //       a.onerror  = e => window.ReactNativeWebView.postMessage('error:' + e.message);
//   //       const tryPlay = () => a.play().catch(() => setTimeout(tryPlay, 500));
//   //       tryPlay();
//   //     </script>
//   //   </body>
//   // </html>`;
...
//     if (!text?.trim()) return;
//     try {
//       const streamUrl = `${API_BASE_URL}/ai/tts?text=${encodeURIComponent(
//         text,
//       )}`;
...
//           const a=document.getElementById('tts');
//           a.volume=1.0;
//           a.onended=()=>window.ReactNativeWebView.postMessage('done');
//           const play=()=>a.play().catch(()=>setTimeout(play,400));
//           play();
//         </script>
//       </body></html>`;

//       globalTtsRef.current?.current?.injectJavaScript(
//         `document.body.innerHTML = \`${html.replace(/`/g, '\\`')}\`; true;`,
//       );
//     } catch (err) {
