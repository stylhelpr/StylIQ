...
export default function ShoppingCollectionsScreen({navigate, route}: Props) {
  ...
  // ✅ FINAL — Share both text & clickable links across iOS/Android
  const handleShareCollection = async (collection: any) => {
    if (!collection || collection.items.length === 0) {
      Alert.alert('Error', 'Cannot share empty collection');
      return;
    }

    // Build message
    let shareMessage = `✨ Check out my collections: ${collection.name}\n\n`;
    if (collection.description) {
      shareMessage += `${collection.description}\n\n`;
    }

    collection.items.forEach((item: any, index: number) => {
      let url = item.url?.trim() || '';
      if (url && !/^https?:\/\//i.test(url)) url = 'https://' + url;
      shareMessage += `${index + 1}. ${item.title || 'View item'}\n${url}\n\n`;
    });

    const firstItemUrl =
      collection.items[0]?.url && /^https?:\/\//i.test(collection.items[0].url)
        ? collection.items[0].url
        : `https://${collection.items[0]?.url || ''}`;

    try {
      if (Platform.OS === 'ios') {
        await Share.share({
          title: `My Collection: ${collection.name}`,
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <View style={{flexDirection: 'row', gap: 12}}>
            <AppleTouchFeedback
              onPress={() => handleShareCollection(selectedCollection)}>
              <MaterialIcons
                name="share"
                size={24}
                color={theme.colors.primary}
...
//     // Build message
//     let shareMessage = `✨ Check out my wishlist: ${collection.name}\n\n`;
//     if (collection.description) {
//       shareMessage += `${collection.description}\n\n`;
//     }
...
//       if (Platform.OS === 'ios') {
//         await Share.share({
//           title: `My Wishlist: ${collection.name}`,
//           message: shareMessage,
//           url: firstItemUrl,
//         });
//       } else {
...
//     // Build message with clickable URLs
//     let shareMessage = `Check out my wishlist: ${collection.name}\n\n`;
...
//         url = 'https://' + url;
//       }
//       shareMessage += `${index + 1}. ${item.title}\n${url}\n\n`;
//     });

//     // Get the first item's URL for the url parameter
//     const firstItemUrl =
//       collection.items.length > 0
//         ? collection.items[0].url.startsWith('http')
//           ? collection.items[0].url
//           : 'https://' + collection.items[0].url
//         : '';
...
//       await Share.share({
//         message: shareMessage,
//         url: firstItemUrl, // Makes the first item's link clickable in iOS
//       });
//     } catch (error) {
...
//     // Get the first item's URL for the url parameter
//     const firstItemUrl = collection.items.length > 0
//       ? (collection.items[0].url.startsWith('http')
//           ? collection.items[0].url
//           : 'https://' + collection.items[0].url)
//       : '';
