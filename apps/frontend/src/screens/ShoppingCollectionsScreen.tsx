...
export default function ShoppingCollectionsScreen({navigate, route}: Props) {
  ...
  const handleShareCollection = async (collection: any) => {
    ...
    // Get the first item's URL as the clickable link
    const firstItemUrl = collection.items.length > 0 ? collection.items[0].url : '';

    try {
      await Share.share({
        message: shareMessage,
        url: firstItemUrl, // This makes the link clickable in iOS
      });
    } catch (error) {
