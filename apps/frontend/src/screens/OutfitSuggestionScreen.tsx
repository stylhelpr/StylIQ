...
export default function OutfitSuggestionScreen({navigate}: Props) {
  ...
  // ⬇️ Gate “Generate Outfit” on prompt text
  ...
  const handleGenerate = () => {
    if (!userId) return;
    if (!canGenerate) return; // ⬅️ ignore taps if prompt is empty
  ...
  // ⬇️ Only show cards after an outfit exists
          ...
          <View style={globalStyles.centeredSection}>
            <ScrollView
              ...
              <OutfitTuningControls
                ...
                // ⬇️ NEW: disable Generate if no prompt entered
              ...
              {/* ⬇️ Cards & actions are HIDDEN until an outfit is generated */}
              {hasOutfit && (
                <>
            ...
            {/* Modals — also gated by hasOutfit to avoid empty content */}
            {hasOutfit && (
              <>
                <WhyPickedModal
...
//   // ⬇️ NEW: Only show cards after an outfit exists
//   const hasOutfit = useMemo(() => {
//     const o: any = current;
//     const items = Array.isArray(o?.outfits?.[0]?.items)
//       ? o.outfits[0].items
