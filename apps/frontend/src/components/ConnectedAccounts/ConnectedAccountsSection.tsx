// src/components/sections/ConnectedAccountsSection.tsx
...
import {
  ...
  SocialPlatform,
} from '../../../../../store/connectedAccountsStore';
import {useUUID} from '../../context/UUIDContext';
import {useOAuthConnect} from '../../hooks/useOAuthConnect';

// ✅ SVG brand icons (downloaded from https://simpleicons.org)
import Instagram from '../../assets/icons/social/instagram.svg';
import TikTok from '../../assets/icons/social/tiktok.svg';
import Pinterest from '../../assets/icons/social/pinterest.svg';
import Threads from '../../assets/icons/social/threads.svg';
import XIcon from '../../assets/icons/social/x.svg';
import Facebook from '../../assets/icons/social/facebook.svg';
import LinkedIn from '../../assets/icons/social/linkerd.svg';

// Map SVG components for easier access
const ICON_MAP: Record<SocialPlatform, React.ReactNode> = {
  instagram: <Instagram width={28} height={28} />,
  tiktok: <TikTok width={28} height={28} />,
  pinterest: <Pinterest width={28} height={28} />,
  threads: <Threads width={28} height={28} />,
  twitter: <XIcon width={28} height={28} />,
  facebook: <Facebook width={28} height={28} />,
  linkedin: <LinkedIn width={28} height={28} />,
};
...
const SOCIAL_PLATFORMS: Array<{
  ...
  {platform: 'twitter', label: 'X', color: '#000000'},
...
export default function ConnectedAccountsSection({
  ...
  const {accounts, loading, fetchConnectedAccounts, disconnectAccount} =
    useConnectedAccountsStore();
  const {initiateOAuthFlow, error: oauthError} = useOAuthConnect();

  // Fetch connected accounts on mount
  useEffect(() => {
    if (userId) fetchConnectedAccounts(userId);
  ...
  useEffect(() => {
    const handleDeepLink = ({url}: {url: string}) => {
      ...
      if (parts[0] === 'oauth' && parts[1] === 'callback' && userId) {
        setTimeout(() => fetchConnectedAccounts(userId), 500);
      }
    };
  ...
  const handleConnect = async (platform: SocialPlatform) => {
    try {
      ...
      if (onConnect) return await onConnect(platform);
      if (!userId) throw new Error('User ID not found');
      await initiateOAuthFlow(platform, userId);
    } catch (err) {
  ...
  const handleDisconnect = async (platform: SocialPlatform) => {
    try {
      setLocalLoading(platform);
      if (onDisconnect) await onDisconnect(platform);
      else if (userId) await disconnectAccount(userId, platform);
    } catch (err) {
    ...
    <Animatable.View
      ...
      <Text
        style={[
          ...
          {marginBottom: 16, color: theme.colors.foreground, fontSize: 13},
        ]}>
      ...
      {loading ? (
        ...
        <View style={styles.gridContainer}>
          {SOCIAL_PLATFORMS.map((sp, i) => {
            const account = accounts[sp.platform];
            const isConnected = account?.isConnected || false;
            const isLoading = localLoading === sp.platform;
            ...
            return (
              <Animatable.View
                ...
                <AppleTouchFeedback
                  onPress={() =>
                    isConnected
                      ? handleDisconnect(sp.platform)
                      : handleConnect(sp.platform)
                  }
                  disabled={isLoading}
                  ...
                  style={[
                    styles.accountCard,
                    {
                      ...
                      borderColor: isConnected ? sp.color : theme.colors.muted,
                      borderWidth: 1.5,
                    },
                  ]}>
                  {isLoading && (
                  ...
                  {/* ✅ SVG icon render */}
                  <View style={{marginBottom: 8}}>{ICON_MAP[sp.platform]}</View>
...
//   {platform: 'twitter', label: 'X', icon: 'x', color: '#000000'},
...
//         console.log(
//           `[ConnectedAccounts] OAuth callback received for ${platform}`,
//         );
