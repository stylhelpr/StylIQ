...
export default function ShoppingAssistant({
  ...
  useEffect(() => {
    if (aiSuggestions.length > 0) {
      setCurrentSuggestion(aiSuggestions[0]);
      aiSuggestionIndex.current = 0;
    }
  }, [aiSuggestions]);

  // 🔄 Continuous smooth bobbing motion
  useEffect(() => {
    const animate = () => {
      bounce.value = withRepeat(
        ...
        -1,
        true, // ✅ mirror direction (removes the reset snap)
      );
    };
    animate();
  }, []);
  ...
  const getNextSuggestion = (): Suggestion => {
    ...
    if (aiSuggestions.length > 0) {
      aiSuggestionIndex.current =
        (aiSuggestionIndex.current + 1) % aiSuggestions.length;
          ...
          <View
            ...
            <Image
              ...
              style={{
                width: '100%',
                height: '100%',
                borderRadius: 75 / 2, // ✅ match outer radius for clean mask
                resizeMode: 'cover',
              }}
            />
          </View>

          {/* 👜 Sleek high-fashion bag badge */}
          <View
            style={{
              position: 'absolute',
              bottom: -4,
...
//       // fetchAISuggestions(); // DISABLED - API costs Turn this back on to re-enable AI suggestions
//     }
//   }, [userId]);
