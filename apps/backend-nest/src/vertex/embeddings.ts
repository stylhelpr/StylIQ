...
// Get an OAuth2 access token for Vertex API calls
...
/**
 * 🔹 Embed text with Vertex AI
 * Uses model: text-embedding-004 (512 dims)
 */
export async function embedText(text: string): Promise<number[]> {
  ...
  const resp = await fetch(url, {
    ...
    body: JSON.stringify({
      // 👇 Follows Vertex REST predict schema
      instances: [
        {
          content: text,
          parameters: { outputDimensionality: 512 }, // match Pinecone config
        },
      ],
    }),
  });
...
/**
 * 🔹 Embed image with Vertex AI
 * Uses model: multimodalembedding@001 (512 dims)
 */
export async function embedImage(gcsUri: string): Promise<number[]> {
  ...
  const resp = await fetch(url, {
    ...
    body: JSON.stringify({
      instances: [
        {
          image: { gcsUri }, // 👈 expects a GCS path, not public https URL
