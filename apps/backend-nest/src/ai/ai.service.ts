...
-------------------------------------------------------------*/
    ...
    //     ${rules}
    //     ${profileConstraints}
    //     - ALWAYS output a complete outfit with distinct Top, Bottom, Shoes, and (if appropriate) Outerwear and Accessories.
    // - DO NOT skip any pieces because the user already owns them; duplicates are fine.
    // - Each outfit piece must include category, item, color, and fit.
...
// # IMAGE INSPIRATION RULES
// • Even if wardrobe items are reused, include one representative image for visual presentation.
// • Treat the uploaded image as the anchor inspiration.
// • Maintain at least one clear visual or color element (pattern, tone, or silhouette) from that image.
// • Keep roughly the same aesthetic DNA — only adapt fabrics, fits, and colors to the user's profile.
// • Balance personalization with faithfulness to the original image; it should feel clearly inspired, not random.
