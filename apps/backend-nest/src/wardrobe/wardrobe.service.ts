...
export class WardrobeService {
  ...
  async generateOutfits(
    ...
    try {
      ...
      // 🔧 Adjust lockedIds based on user refinement text
      if (lockedIds?.length && refinement) {
        const lower = refinement.toLowerCase();

        // 🩳 If user says "keep the shorts" → only keep bottoms
        if (/keep .*shorts?/.test(lower)) {
          const before = [...lockedIds];
          lockedIds = lockedIds.filter((id) => {
            ...
            return (
              main === 'bottoms' ||
              /\bshorts?\b/.test(sub) ||
              /\bshorts?\b/.test(item.label.toLowerCase())
            );
          });
          console.log(
            '🔓 User said keep shorts → removed non-bottom locked items:',
            before.filter((x) => !lockedIds.includes(x)),
            'kept:',
            lockedIds,
          );
        }

        // 👟 If user says "keep the shoes" → only keep shoes
        if (/keep .*shoes?/.test(lower)) {
          const before = [...lockedIds];
          lockedIds = lockedIds.filter((id) => {
            const item = catalog.find((c) => c.id === id);
            if (!item) return false;
            return (item.main_category ?? '').toLowerCase() === 'shoes';
          });
          console.log(
            '🔓 User said keep shoes → removed non-shoe locked items:',
            before.filter((x) => !lockedIds.includes(x)),
            'kept:',
            lockedIds,
          );
        ...
        // 👖 If user asks for black pants, try to find one and prefer it
        if (/black (pants?|trousers?)/.test(lower)) {
          const blackPants = catalog.find(
            (c) =>
              (c.main_category ?? '').toLowerCase() === 'bottoms' &&
              /black/i.test(`${c.color ?? ''} ${c.label ?? ''}`),
          );
          if (blackPants) {
            console.log(
              '🎯 User requested black pants — found:',
              blackPants.id,
            );
          } else {
            console.log(
              '⚠️ User requested black pants but none found in catalog.',
            );
          }
        }

        // 👕 If user asks for blue shirt, try to find one and prefer it
        if (/blue (shirt|top|tee|t-?shirt)/.test(lower)) {
          const blueTop = catalog.find(
            (c) =>
              (c.main_category ?? '').toLowerCase() === 'tops' &&
              /blue/i.test(`${c.color ?? ''} ${c.label ?? ''}`),
          );
          if (blueTop && !lockedIds.includes(blueTop.id)) {
            lockedIds.push(blueTop.id);
            console.log('🔒 Locked blue shirt:', blueTop.id);
          }
        }
      }

      // 🔓 Auto-unlock conflicting locked items if user wants something different
      // if (lockedIds.length && refinement) {
      //   const lowerRef = refinement.toLowerCase();
      //   const changeWords =
      //     /\b(instead|swap|change|different|other|replace|not those|new)\b/;
      ...
      //     // detect slot keywords
      //     const slotKeyword =
      //       slotMap[main as keyof typeof slotMap] ?? (main || sub || lbl || '');

      //     const mentioned = new RegExp(
      //       `\\b(${slotKeyword}|${sub}|${main})\\b`,
      //       'i',
      //     ).test(lowerRef);
      //     const wantsChange = changeWords.test(lowerRef);

      //     // Unlock if user explicitly mentioned this slot and wants a change
      //     if (mentioned && wantsChange) {
      //       console.log(
      //         '🔓 Unlocking locked item due to refinement change request →',
      //         id,
      //         item.label,
      //       );
      //       return false; // remove from lockedIds
      //     }
      //     return true;
      //   });

      //   const removed = before.filter((id) => !lockedIds.includes(id));
      //   if (removed.length) {
      //     console.log('🔓 Auto-unlocked items:', removed);
      //   }
      // }

      // 🔓 Auto-unlock conflicting locked items based on refinement prompt
      // if (lockedIds.length && refinement) {
      //   const lower = refinement.toLowerCase();

      //   // If user wants to change shoes, unlock any locked shoes
      //   if (
      //     /\b(shoes?|sneakers?|loafers?|boots?|oxfords?|derbys?|dress shoes?)\b/.test(
      //       lower,
      //     ) &&
      //     /\b(instead|swap|change|different|other|replace|not those|new)\b/.test(
      //       lower,
      //     )
      //   ) {
      //     const before = [...lockedIds];
      //     lockedIds = lockedIds.filter((id) => {
      //       const item = catalog.find((c) => c.id === id);
      //       return item && (item.main_category ?? '').toLowerCase() !== 'shoes';
      //     });
      //     console.log(
      //       '🔓 Unlocking locked shoes due to refinement → removed:',
      //       before.filter((id) => !lockedIds.includes(id)),
      //     );
      //   }

      //   // If user asks for shorts, unlock locked bottoms that are not shorts
      //   if (/\bshorts?\b/.test(lower)) {
      //     const before = [...lockedIds];
      //     lockedIds = lockedIds.filter((id) => {
      //       const item = catalog.find((c) => c.id === id);
      //       if (!item) return true;
      ...
      //       const lbl = (item.label ?? '').toLowerCase();
      //       return !(
      //         item.main_category === 'Bottoms' && !/\bshorts?\b/.test(sub + lbl)
      //       );
      //     });
      //     console.log(
      //       '🔓 Unlocking locked bottoms due to refinement → removed:',
      //       before.filter((id) => !lockedIds.includes(id)),
      //     );
      //   }
      // }

      // 🔓 If the refinement asks to change/replace shoes, unlock any locked SHOES
      if (lockedIds.length && refinement) {
        const wantsShoeSwap =
          /\b(shoes?|sneakers?|loafers?)\b/i.test(refinement) &&
          /\b(instead|swap|change|different|other|replace|not those)\b/i.test(
            refinement,
          );
        const wantsSpecificRedShoes = /\bred shoes?\b/i.test(refinement);

        if (wantsShoeSwap || wantsSpecificRedShoes) {
          const before = new Set(lockedIds);
          const unlocked = catalog
            .filter(
              (c) =>
                before.has(c.id) &&
                (c.main_category ?? '').toLowerCase() === 'shoes',
            )
            .map((c) => c.id);

          if (unlocked.length) {
            lockedIds = lockedIds.filter((id) => !unlocked.includes(id));
            console.log(
              '🔓 Unlocking locked shoes due to refinement → removed:',
              unlocked,
              'remaining lockedIds:',
      ...
      // 🔒 Relock explicit targets mentioned in the refinement (color + slot)
      if (refinement) {
        const lcRef = refinement.toLowerCase();
        const slotOf = (c?: CatalogItem) =>
          (c?.main_category ?? '').toLowerCase();
        const textOf = (c: CatalogItem) =>
          `${c.color ?? ''} ${c.color_family ?? ''} ${c.label ?? ''}`.toLowerCase();
        const findFirst = (pred: (c: CatalogItem) => boolean) =>
          catalog.find(pred)?.id;
        const removeLockedSlot = (slot: 'tops' | 'bottoms' | 'shoes') => {
          lockedIds = lockedIds.filter((id) => {
            const it = catalog.find((x) => x.id === id);
            return it ? slotOf(it) !== slot : true;
          });
        };

        const wants = (color: RegExp, slot: RegExp) =>
          color.test(lcRef) && slot.test(lcRef); // order-agnostic

        const shortsSlot = /(shorts?|bottoms?)/i;
        const shoesSlot = /(shoes?|sneakers?|trainers?)/i;
        const topsSlot = /(shirt|top|tee|t-?shirt|polo)/i;
        // Grey/Gray shorts → (re)lock a grey shorts bottom
        if (wants(/grey|gray/i, shortsSlot)) {
          const id = findFirst(
            (c) =>
              slotOf(c) === 'bottoms' &&
              (c.subcategory ?? '').toLowerCase().includes('short') &&
              /grey|gray/i.test(textOf(c)),
          );
          if (id) {
            removeLockedSlot('bottoms');
            lockedIds.push(id);
          }
        }
        // Red or Green shoes → (re)lock matching shoes
        if (wants(/red/i, shoesSlot) || wants(/green/i, shoesSlot)) {
          const colorRe = /red/.test(lcRef) ? /red/i : /green/i;
          const id = findFirst(
            (c) => slotOf(c) === 'shoes' && colorRe.test(textOf(c)),
          );
          if (id) {
            removeLockedSlot('shoes');
            lockedIds.push(id);
          }
        }

        // Blue (incl. navy/light blue) shirt/top → (re)lock a blue top
        if (wants(/blue|navy|light\s*blue/i, topsSlot)) {
          const id = findFirst(
            (c) => slotOf(c) === 'tops' && /(blue|navy)/i.test(textOf(c)),
          );
          if (id) {
            removeLockedSlot('tops');
            lockedIds.push(id);
          }
        }

        console.log('🔒 Final lockedIds after refinement merge:', lockedIds);
      }
...
//       // Build locked section for prompt
//       let lockedSection = '';
//       if (locked.length) {
//         lockedSection = `
// LOCKED ITEMS (MANDATORY):
// - These locked items MUST appear in every single outfit.
// - Do NOT remove, swap, or omit them.
// - Only choose additional non-locked items to complete the outfit.
// ${locked.map((l) => `${l.index}. ${l.label}`).join('\n')}
// `;
//       }
...
//       const locked = reranked.filter((it) => (it as any).__locked);
//       const unlocked = reranked.filter((it) => !(it as any).__locked);
//       const workingCatalog = [...locked, ...unlocked];

//       // 5) Prompt (use effectiveQuery; also include refinement explicitly)
//       const catalogLines = workingCatalog
//         .map((c) => `${c.index}. ${c.label}`)
//         .join('\n');
...
//             (c) => (c as any).__locked && !existingIds.has(c.id),
//           );
//           return { ...o, items: [...lockedItems, ...o.items] };
//         });
//       }
