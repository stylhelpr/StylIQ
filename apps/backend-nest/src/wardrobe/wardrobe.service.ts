...
//       // Check toggle
//       const disableFeedback = process.env.DISABLE_FEEDBACK === '1';
//       console.log('[FEEDBACK] DISABLE_FEEDBACK =', disableFeedback);

//       if (process.env.DISABLE_FEEDBACK !== '1') {
//         feedbackRows = await this.fetchFeedbackRows(userId);
//         feedbackRules = compileFeedbackRulesFromRows(feedbackRows);
...
//           console.debug('[FEEDBACK] skipped — DISABLE_FEEDBACK=1');
//         }
//       }
...
//       // 3b) Apply user feedback (dislikes/bans) before reranking/LLM
//       const feedbackRows = await this.fetchFeedbackRows(userId);
//       const feedbackRules = compileFeedbackRulesFromRows(feedbackRows);
...
//                   (r as any) &&
//                   (function rule(it: any) {
//                     // inline eval to reuse logic from file
//                     switch (r.kind) {
//                       case 'excludeItemIds':
//                         return !!(it.id && r.item_ids.includes(it.id));
//                       default:
//                         return false; // keep debug light
//                     }
//                   })(it)
//                 );
//               } catch {
...
//       // Enforce (strong, then soften if list would get too small)
//       catalog = applyFeedbackFilters(catalog, feedbackRules, {
//         minKeep: 6, // keep at least 6 items visible
//         softenWhenBelow: true, // if too aggressive, fall back to item-id bans only
//       });
...
//       // (keep) catalog = applyFeedbackFilters(catalog, feedbackRules, { ... });
...
//       // ──────────────────────────────────────────────
//       // Build userPrefs from both rules + DB per-item scores
//       // ──────────────────────────────────────────────

//       // 1) Build soft prefs from rules (existing logic)
//       const userPrefs = buildUserPrefsFromRules(catalog, feedbackRules);
...
//       // 4) Proceed to rerank with enriched userPrefs
//       const reranked = rerankCatalogWithContext(catalog, constraints, {
//         userStyle: opts?.userStyle,
//         weather: opts?.weather,
//         weights: tunedWeights,
//         useWeather: opts?.useWeather,
//         userPrefs, // ✅ now includes both rules + DB prefs
//       });
...
//         longW.styleWeight = 0.35;
//       }
//       const tunedWeights = fromLongWeights(longW, incoming);

//       // Build soft per-item nudges from rules (for the rerank totals/logs)
//       const userPrefs = buildUserPrefsFromRules(catalog, feedbackRules);
...
//         weights: tunedWeights,
//         useWeather: opts?.useWeather,
//         userPrefs, // ✅ this makes feedbackRaw/feedbackScaled non-zero
//       });
...
//       let parsed: any = {};
//       try {
//         parsed = r.notes ? JSON.parse(r.notes) : {};
//       } catch {
//         parsed = {};
//       }

//       return {
//         type: r.rating ?? null, // "like" or "dislike"
//         text: parsed.reason ?? null, // free-text reason
//         tags: parsed.tags ?? null, // user tags
//         item_id: parsed.selected_item_ids
//           ? Array.isArray(parsed.selected_item_ids)
//             ? parsed.selected_item_ids
//             : Object.keys(parsed.selected_item_ids)
//           : null,
//         payload: parsed, // keep full JSON
//         context_query: null,
//       };
//     });
//   }
...
//       // Normalize weights → tweak → convert back to original shape
//       const incoming = opts?.weights ?? DEFAULT_CONTEXT_WEIGHTS;
//       const longW = toLongWeights(incoming);
//       if (needUpscale && longW.styleWeight > 0.35) {
//         longW.styleWeight = 0.35; // prevent prefs from overriding dress code
//       }
//       const tunedWeights = fromLongWeights(longW, incoming);
...
//       // NEW: minimal personalization using per-item prefs
//       // ──────────────────────────────────────────────────────────────
//       const withIds = outfits.map((o, idx) => ({
//         ...o,
//         outfit_id: randomUUID(),
//       }));
//       const ids = Array.from(
...
//         );
//       if (needUpscale) {
//         catalog = catalog.filter((c) => !this.isUltraCasualForUpscale(c));
//       }
