...
/** ───────────────────────────────────────────────────────────────────────────
 * NEW: Context blend (constraints + user style + weather + (optional) boosts)
 * ---------------------------------------------------------------------------
 */
 ...
 * ───── NEW: accepts optional per-item boosts (from FeedbackService.getUserItemBoosts)
 */
export function rerankCatalogWithContext<T extends CatalogItemLite>(
  ...
  opts?: {
    ...
    // ───── NEW: boosts hook
    itemBoosts?: ItemBoostMap;
    boostAlpha?: number; // default 0.15
  },
): T[] {
  const W = normalizeWeights(opts?.weights);
  const alpha = typeof opts?.boostAlpha === 'number' ? opts!.boostAlpha : 0.15;
  const boosts = opts?.itemBoosts;
  ...
  const scored = catalog.map((item, i) => {
    ...
    // ───── NEW: apply learned boost per item id (if provided)
    const id = text((item as any).id);
    const boost = boosts ? boosts[id] || 0 : 0;
    const final = total + alpha * boost;

    // 🪵 Debug log (keep or remove)
    console.log(
      `[RERANK] ${item.label ?? id}`,
      `constraints=${constraintsScore.toFixed(2)}`,
      `style=${styleScore.toFixed(2)}`,
      `weather=${weatherScore.toFixed(2)}`,
      boosts ? `boost=${boost}` : `boost=0`,
      `→ total=${total.toFixed(2)} final=${final.toFixed(2)}`,
