...
// const RX_HOODIE = /\bhoodie\b/i;
// const RX_WINDBREAKER = /\b(windbreaker|shell|track\s*top)\b/i;
// const RX_SWIM = /\b(swim|bikini|trunks|boardshorts|one(-|\s)?piece)\b/i;
// const RX_SANDAL = /\b(sandals?|flip-?flops?|slides?|espadrilles?)\b/i;
// const RX_BLAZER = /\b(blazer|sport\s*coat|suit\s*jacket)\b/i;
...
// /** Allowlist for beach/pool. */
// function allowBeach(item: CatalogItem): boolean {
//   const main = mainOf(item);
//   const sub = subOf(item);
//   const lbl = labelOf(item);

//   if (main === 'swimwear') return true;
//   if (main === 'shoes' && RX_SANDAL.test(sub)) return true;

//   // Lightweight summer tops/bottoms (linen, tanks, resort shirts)
//   const summerTop =
//     main === 'tops' &&
//     (/\blinen\b/i.test(lbl) || /\b(tank|camp\s*shirt|resort)\b/i.test(sub));
//   const summerBottom =
//     main === 'bottoms' && (RX_SHORTS.test(sub) || /\blinen\b/i.test(lbl));
//   if (summerTop || summerBottom) return true;

//   // Light outer layer for breezy evenings
//   if (main === 'outerwear' && /\b(lightweight|linen|unstructured)\b/i.test(lbl))
//     return true;
...
//   const main = mainOf(item);
//   const sub = subOf(item);
//   if (RX_BLAZER.test(sub)) return true;
//   if (main === 'shoes' && (RX_DRESS_SHOE.test(sub) || /boots?/.test(sub)))
//     return true;
//   if (RX_JEANS.test(sub)) return true;
//   return false;
// }
