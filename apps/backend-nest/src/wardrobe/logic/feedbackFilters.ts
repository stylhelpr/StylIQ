...
// // Your real feedback table schema
// export type OutfitFeedbackRow = {
//   id: string;
//   request_id: string;
//   user_id: string;
//   rating: string | null; // "like" | "dislike"
//   tags: string[] | null;
//   notes: string | null;
//   outfit_json: any | null;
//   created_at: string;
// };
...
// // ─────────────────────────────────────────────────────────────
// // Compile rules from outfit_feedback rows
// // ─────────────────────────────────────────────────────────────
...
//     // tags
//     for (const tag of row.tags ?? []) rules.push(...extractRulesFromText(tag));
//     // notes
//     if (row.notes) rules.push(...extractRulesFromText(row.notes));

//     // dislike → ban outfit items
//     if (row.rating?.toLowerCase() === 'dislike') {
//       try {
//         const outfit =
//           typeof row.outfit_json === 'string'
//             ? JSON.parse(row.outfit_json)
//             : row.outfit_json;
//         if (outfit?.items?.length) {
//           const ids = outfit.items.map((it: any) => it?.id).filter(Boolean);
//           if (ids.length) rules.push({ kind: 'excludeItemIds', item_ids: ids });
//         }
//       } catch (e) {
//         console.warn('⚠️ Failed to parse outfit_json in feedback row', e);
//       }
//     }
//   }
...
// export type FeedbackFilterOptions = {
//   /** If strong filtering leaves fewer than this, fall back to a softer pass. */
//   minKeep?: number;
//   /** When true, attempt a soft filter (item_id bans only) if strong filtering drops below minKeep. */
//   softenWhenBelow?: boolean;
// };
...
//       return (
//         m === 'shoes' ||
//         /\b(sneakers?|trainers?|loafers?|boots?|heels?|oxfords?|derbys?|dress\s*shoes?|sandals?|slides?|espadrilles?)\b/i.test(
//           s,
//         )
//       );
//     case 'tops':
...
//       return (
//         m === 'outerwear' ||
//         /\b(blazer|sport\s*coat|suit\s*jacket|jacket|coat|parka|trench|overcoat|windbreaker)\b/i.test(
//           s,
//         )
//       );
//     case 'accessories':
...
// function itemHasColor(it: CatalogItem, color: NormalizedColor): boolean {
//   const c = lc(it.color);
//   const f = lc(it.color_family);
//   const lbl = labelOf(it);
//   const rx = COLOR_CANON[color];
//   return rx.test(c) || rx.test(f) || rx.test(lbl);
// }
...
//       rules.push(...extractRulesFromText(row));
//       continue;
//     }
...
//     if (row.item_id)
//       rules.push({ kind: 'excludeItemIds', item_ids: [row.item_id] });
//     for (const tag of tags) rules.push(...extractRulesFromText(tag));
//     if (text) rules.push(...extractRulesFromText(text));

//     const p = row.payload;
//     if (p?.ban?.item_ids?.length)
//       rules.push({ kind: 'excludeItemIds', item_ids: p.ban.item_ids });
//     if (p?.ban?.brand)
//       rules.push({
//         kind: 'excludeBrand',
//         brand: lc(String(p.ban.brand)),
//         category: p.ban.category
//           ? normalizeCategoryText(String(p.ban.category))
//           : undefined,
//       });
//     if (p?.ban?.color) {
//       const color = normalizeColor(p.ban.color);
//       if (color !== 'unknown') {
//         rules.push(
//           p.ban.category
//             ? {
//                 kind: 'excludeColorOnCategory',
//                 color,
//                 category: normalizeCategoryText(String(p.ban.category)),
//               }
//             : { kind: 'excludeColor', color },
//         );
//       }
//     }
//   }
...
//   return out; // ✅ ensure we always return
// }
...
//     case 'excludeBrand': {
//       const brand = lc(it.brand);
//       const hit = brand && brand.includes(lc(rule.brand));
//       return hit
//         ? rule.category
//           ? itemIsCategory(it, rule.category)
//           : true
...
//       return itemIsCategory(it, rule.category) && itemHasColor(it, rule.color);
//     }
//     case 'excludeColor':
//       return itemHasColor(it, rule.color);
//     case 'excludeSubstring': {
