// apps/backend-nest/src/wardrobe/logic/constraints.ts
// parse user request constraints (moved from service)
...
export function parseConstraints(q: string): ParsedConstraints {
  ...
  // NEW: detect gym/activewear intent
  const wantsGym =
    /\b(gym|work\s*out|workout|fitness|exercise|train|athletic)\b/i.test(s);
